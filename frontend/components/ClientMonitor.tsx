...
export default function ClientMonitor({ clientId }: ClientMonitorProps) {
  ...
  const [clientName, setClientName] = useState<string>(`Client ${clientId}`);
  ...
  const fetchSimulation = useCallback(
    async (leak: boolean) => {
      ...
      try {
        ...
        const response = await fetch(
          `${baseUrl}/clients/${clientId}?company_id=${COMPANY_ID}&leak=${leak}&profile=${customerProfile}`,
          {
            cache: "no-store",
          }
        );
    ...
    [clientId, customerProfile, leakEventTime]
  );
  ...
  const handleReset = useCallback(() => {
    const resetSimulation = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${baseUrl}/clients/${clientId}/reset?company_id=${COMPANY_ID}`,
          {
            method: "POST",
            headers: {
        ...
        await (response.json() as Promise<ResetResponse>);
  ...
  return (
    <main
      className={`relative min-h-screen overflow-hidden px-4 pb-12 pt-10 sm:px-8 lg:px-12 ${severity === "CRITICAL" ? "critical-scene" : ""}`}
    >
      ...
      <div className="mx-auto max-w-6xl animate-fadeIn space-y-6">
        <header className="rounded-2xl border border-cyan-400/20 bg-slate-950/80 p-6 shadow-[0_0_80px_-25px_rgba(6,182,212,0.4)] backdrop-blur">
          <div className="mb-4 flex items-center justify-between gap-3">
            <Link
              href="/"
              className="rounded-lg border border-slate-700 bg-slate-900/70 px-3 py-1 text-xs text-slate-300 hover:border-cyan-500"
            >
              Back to Clients
            </Link>
            <div className="rounded-full border border-slate-700 bg-slate-900/70 px-3 py-1 text-xs text-slate-400">
              {modeTag}
            </div>
          </div>
          ...
          <p className="mt-2 text-sm text-slate-300 sm:text-base">{clientAddress}</p>
        </header>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <section className="flex flex-col items-center justify-center rounded-2xl border border-slate-700 bg-slate-950/70 p-6 backdrop-blur">
            ...
            <span
              className={`mt-3 rounded-full px-4 py-1 text-sm font-semibold ${leakBadgeStyle}`}
            >
              {leakLabel}
            </span>
          </section>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {(
            [
              ...
              {
                label: "Runtime",
                value: latestData?.runtime ?? "-",
                unit: "min",
                color: "text-amber-400",
              },
              {
                label: "Superheat",
                value: latestData?.superheat ?? "-",
                unit: "F",
                color: "text-violet-400",
              },
              {
                label: "Delta-T",
                value: latestData?.delta_t ?? "-",
                unit: "F",
                color: "text-sky-400",
              },
            ] as const
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <section className="rounded-2xl border border-rose-500/25 bg-rose-950/25 p-5 backdrop-blur">
            ...
            <p className="text-xs text-rose-200/80">
              per month in wasted energy if unresolved
            </p>
        ...
        <section className="rounded-2xl border border-cyan-400/20 bg-slate-950/80 p-6 shadow-[0_0_60px_-20px_rgba(6,182,212,0.25)] backdrop-blur">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Enter your phone or email and we will text you the moment we detect a problem.
          </p>
